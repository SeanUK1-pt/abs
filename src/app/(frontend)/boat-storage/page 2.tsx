...
export default async function BoatStoragePage() {
  ...
  return (
    <>
      ...
      {/* Breadcrumb-style page title — matches old site's layout (no big hero banner on this page) */}
      <section className={styles.pageHeader}>
        <div className="container">
          <h1>Storage Packages</h1>
        </div>
      </section>
      ...
      <div className="container">
          ...
          <div className={styles.introPhotos}>
            <div className={styles.introPhoto}>
              <Image
                ...
                alt="Boats stored on trailers inside Algarve Boat Sales' indoor storage facility"
                fill
            ...
            <div className={styles.introPhoto}>
              <Image
                src="/media/yamm-17-02.jpg"
                alt="Triple outboard engines on a boat at the marina"
                fill
                sizes="(max-width: 768px) 100vw, 300px"
              />
            </div>
        ...
        {/* Full-width promo strip with CTA — matches old site, uses the same yamm-17-02 photo at wide crop */}
        <section className={styles.promoStrip}>
          <Image
            src="/media/yamm-17-02.jpg"
            alt="Algarve Boat Sales storage facility exterior"
            fill
            sizes="100vw"
            className={styles.promoStripImg}
          />
          <div className={styles.promoStripCard}>
            <p>Give your boat the care it deserves while in storage</p>
        ...
        {/* Brand strip — 6 individual logos, confirmed against the live site. 2 of 6 already in /public/media; see README for the other 4. */}
        <section className={styles.brandStrip}>
          <div className={styles.brandStripRow}>
            {[
              { name: 'SPX RIB', file: 'spx-rib-logo.png', pending: true },
              { name: 'Yamarin', file: 'yamarin-logo.png', pending: true },
              { name: 'GRAND Inflatable Boats', file: 'grand-logo.png', pending: true },
              { name: 'Vanclaes', file: 'vanclaes_logo-removebg-preview.png', pending: false },
              { name: 'Blue Marine', file: 'bluemarine_Prancheta-1-06.png', pending: false },
              { name: 'Salt-Away', file: 'salt-away-logo.png', pending: true },
            ].map(brand => (
              brand.pending ? (
                <div key={brand.name} className={styles.brandLogoPending} data-pending-asset={brand.file}>
                  {brand.name}
                </div>
              ) : (
                <div key={brand.name} className={styles.brandLogo}>
                  <Image src={`/media/${brand.file}`} alt={brand.name} width={140} height={50} style={{ objectFit: 'contain' }} />
                </div>
              )
            ))}
          </div>
